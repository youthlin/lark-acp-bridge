...
func workspaceACPTraceSkillContent() string {
	return strings.Join([]string{
		...
		"   - `error` 是执行错误。",
		"6. 如果 trace 记录包含顶层 `message_id`，可用它把同一个 ACP session 内的多轮聊天区分开；用户指定某条飞书消息时优先按 `message_id` 过滤。",
		"7. 默认输出高信号摘要：会话路径、用户请求、最终 assistant 回复、关键计划/工具/错误。不要整段粘贴超长 trace；用户要求细节时再展开。",
		"8. 涉及隐私、密钥、token、cookie、app_secret 等敏感内容时只概述，不原样输出。",
		"",
		"## Useful Commands",
		"",
		"```bash",
